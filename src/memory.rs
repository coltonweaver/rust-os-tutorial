...
/// mapped to virtual memory at the passed `physical_memory_offset`. Also, this function must only
/// be called once to avoid aliasing `&mut` references (which is undefined behavior).
